...
use App\Models\ProsesFraksinasi;
use App\Models\BahanFraksinasi;
use App\Models\HasilFraksinasi;
...
class ProsesFraksinasiController extends Controller
{
    ...
    {
        $query = ProsesFraksinasi::with([
            'bahanFraksinasis.produk',
            'bahanFraksinasis.storage',
            'hasilFraksinasis.produk',
            'hasilFraksinasis.storage',
        ])->orderBy('tgl', 'desc');
    ...
    public function store(Request $request)
    {
        ...
        return DB::transaction(function () use ($request) {
            $proses = ProsesFraksinasi::create([
                'tgl'     => $request->tgl,
                'shift'   => $request->shift,
            ...
            foreach ($request->bahan ?? [] as $b) {
                $this->allocateBahan(
                    ...
                    $b['produk_id'],
                    $b['qty'],
                    BahanFraksinasi::class,
                    'proses_fraksinasi_id',
                    $b['storage_id'] ?? null
                );
            }
    ...
    {
        $proses = ProsesFraksinasi::findOrFail($id);
        ...
        return DB::transaction(function () use ($request, $proses) {
            ...
            if ($request->has('bahan')) {
                $oldBahans = BahanFraksinasi::where('proses_fraksinasi_id', $proses->id)->get();
                ...
                BahanFraksinasi::where('proses_fraksinasi_id', $proses->id)->delete();
            ...
            if ($request->has('hasil')) {
                $oldHasils = HasilFraksinasi::where('proses_fraksinasi_id', $proses->id)->get();
                ...
                HasilFraksinasi::where('proses_fraksinasi_id', $proses->id)->delete();
    ...
    public function destroy($id)
    {
        ...
        DB::transaction(function () use ($proses) {
            // Revert bahans
            foreach ($proses->bahanFraksinasis as $ob) {
                $stok = StokProduk::where('produk_id', $ob->produk_id)
                    ->where('storage_id', $ob->storage_id)
                    ->first();
                if ($stok) {
            ...
            // Revert hasils
            foreach ($proses->hasilFraksinasis as $oh) {
                $stok = StokProduk::where('produk_id', $oh->produk_id)
                    ->where('storage_id', $oh->storage_id)
                    ->first();
                if ($stok) {
        ...
        return response()->json(['message' => 'Proses fraksinasi dihapus']);
    }
